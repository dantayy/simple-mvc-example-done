...
// function to retrieve all dogs on request
...
// function for handling creation of a new dog
...
// function for handling aging of a specified dog
const ageDog = (req, res) => {
  ...
  // Call our Dog's static updateAge function.
  // Since this is a static function, we can just call it without an object
  ...
  // For that reason, I gave it an anonymous callback instead of a
  // named function you'd have to go find
  return Dog.updateAge(req.body.name, (err, doc) => {
    // errs, handle them
    if (err) {
      return res.json({ err }); // if error, return it
    }
    ...
    return res.json({ msg: 'Dog age updated' });
  });
};
